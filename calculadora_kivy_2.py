...
# Ajuste para compatibilidade com Gboard no Android 12
Window.softinput_mode = "resize"
Window.clearcolor = (0, 0, 0, 0.9)  # cor de fundo cinza escuro


class CalculadoraKivyFinal(App):
    def build(self):
        self.title = "Calculadora kivy_2"
        self.limite = 0.0
        ...
        # 1. TÍTULO E DATA
        btn_titulo = Button(
            text="CALCULADORA KIVY_2",
            size_hint_y=None,
            height=dp(48),
            background_normal="",
            background_color=(0.17, 0.17, 0.19, 1), # cor de fundo cinza escuro
            bold=True,
            font_size="15sp",
        )
        ...
        self.lbl_data = Label(
            ...
            size_hint_y=None,
            height=dp(20),                # altura ajustada para 20dp
            #halign="right",              # alinhamento horizontal
            #valign="middle",             # alinhamento vertical 
            color=(0.65, 0.55, 0.58, 1), # cor do texto cinza claro
            font_size="16sp",
        )
        ...
        lbl_qt = Label(text="QT:", size_hint=(0.2, 1), halign="center", valign="middle")
        ...
        def focar_proximo(instancia, proximo_campo):
            Clock.schedule_once(lambda dt: setattr(proximo_campo, "focus", True), 0.08)
        ...
        # 3. STATUS DA COMPRA (logo abaixo do formulário)
        ...
        # 4. PAINEL DE BOTÕES
        box_btn = BoxLayout(
            orientation="horizontal", spacing=dp(8), size_hint_y=None, height=dp(28)
        )
        ...
        btn_exc = Button(
            text="Excluir",
            on_press=lambda x: self.remover_produto(),
            background_normal="",
        ...
        btn_sal = Button(
            text="Salvar",
            on_press=lambda x: self.salvar_txt(),
            background_normal="",
        ...
        box_btn.add_widget(btn_add)          # adiciona o botão "Adicionar" ao layout horizontal
        box_btn.add_widget(btn_exc)
        box_btn.add_widget(btn_sal)
        box_btn.add_widget(self.btn_env)
        root.add_widget(box_btn)

        # 5. LISTA DE PRODUTOS (ocupa o restante da tela)
        scroll = ScrollView(size_hint=(1, 1))                        # permite rolagem vertical                            
        self.box_lista = BoxLayout(                                  # layout vertical para os itens da lista 
            orientation="vertical", spacing=dp(4), size_hint_y=None  # altura ajustável conforme o conteúdo
        )
        self.box_lista.bind(minimum_height=self.box_lista.setter("height")) # minimum_height ajusta a altura do BoxLayout conforme o conteúdo 
        scroll.add_widget(self.box_lista)                                   # adiciona o BoxLayout dentro do ScrollView para permitir rolagem 
        root.add_widget(scroll)                                             # adiciona o ScrollView ao layout principal para ocupar o restante da tela

        return root

    # --- FUNÇÕES AUXILIARES ---
    def _focar_campo(self, campo):        # função auxiliar para focar o campo de entrada e abrir o teclado virtual
        campo.focus = True
    ...
    def mostrar_alerta(self, titulo, mensagem):
        ...
        lbl_msg.bind(
            size=lambda inst, val: setattr(inst, "text_size", (val[0] - dp(6), None))
        )
        ...
        popup = Popup(
            title=titulo, content=box, size_hint=(0.8, 0.40), auto_dismiss=False
        )
    ...
    def salvar_txt(self):
        ...
        try:
            ...
            self.mostrar_alerta("Sucesso", f"Nota salva")
            self.limpar_tela()
    ...
    def compartilhar_nota(self):
        ...
        try:
            ...
            try:
                ...
                self.mostrar_alerta(
                    "Info",
                    "Compartilhamento Android indisponível neste ambiente.\n"
                    f"Caminho: {self.caminho_ultimo_arquivo}",
                )
                return
